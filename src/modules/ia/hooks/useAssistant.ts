...
export const useAssistant = (module: string, tabId: string) => {
    ...
    const query = useQuery<Assistant>({
        ...
        queryFn: async (): Promise<Assistant> => {
            ...
            if (mustCreateNew) SessionStorage.clearForceNewAssistant(tabId)

            const cachedId = SessionStorage.getAssistantOpenAiIdForTab(tabId)

            // Cas 1 : assistant déjà mappé localement
            if (cachedId) {
                const assistants = await readAssistants({ module })
            ...
            // Cas 2 : réutilisation possible
            if (!cachedId && !mustCreateNew) {
                const all = await readAssistants()
                const available = all
                    .filter(a => a.module === module)
                    .find(
                        a =>
                            !Object.values(
                                SessionStorage.getAssistantOpenAiIdMapping()
                            ).includes(a.openai_id)
                    )
